...
function makeRandomColsAndRows() {
    rowWidths = Array(cols).fill().map(_ => random(.1, 1))
    const totalRowWidths = sum(rowWidths)
    rowWidths = rowWidths.map(v => v / totalRowWidths * width)
    colHeights = Array(rows).fill().map(_ => random(.1, 1))
    const totalColHeights = sum(colHeights)
    colHeights = colHeights.map(v => v / totalColHeights * height)
}
...
function setupCellByNeighbors(x, y) {
    const leftName = x > 1 ? (x - 1) + ',' + y : cols + ',' + y
    const rightName = x < cols ? (x + 1) + ',' + y : '1,' + y
    const topName = y > 1 ? x + ',' + (y - 1) : x + ',' + rows
    const bottomName = y < rows ? x + ',' + (y + 1) : x + ',1'
    const cell = {
        left: grid[leftName] ? grid[leftName].right : null,
        right: grid[rightName] ? grid[rightName].left : null,
        top: grid[topName] ? grid[topName].bottom : null,
        bottom: grid[bottomName] ? grid[bottomName].top : null
    }
...
function myBezier(x1, y1, x2, y2, x3, y3, x4, y4) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        strokeWeight(strokeThickness * (cellSize / 10) * noise(x / 60, y / 60))
        point(x, y)
    }
}
